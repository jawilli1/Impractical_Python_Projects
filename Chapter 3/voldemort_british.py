...
def prep_words(name, word_list_ini):
    ...
    print("length of initial word list = {}".format(len(word_list_ini)))
    len_name = len(name)
...
def cv_map_filter(name, filtered_cv_map):
    """Remove permutations of words based on unlikely C-V combos."""
    perms = {''.join(i) for i in permutations(name)}
    print("length of original permutations set = {}".format(len(perms)))
    vowels = 'aeiouy'
    filter_1 = set()
...
def letter_pair_filter(filter_2):
    ...
    print("# of choices before filter_3 = {}".format(len(filter_2)))
    filtered = set()
